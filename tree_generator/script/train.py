...
def main():
	...
	for epoch in range(n_epochs):
		...
		for real, _ in tqdm(dataloader):
			...
			for _ in range(crit_repeats):
				...
				#print("AHHHHHH"+ str(crit_fake_pred.shape)+  str(crit_real_pred.shape), str(gp.shape), str(c_lambda))
				crit_loss = get_crit_loss(crit_fake_pred, crit_real_pred, gp, c_lambda)
				#print(crit_loss)
				# Keep track of the average critic loss in this batch
				crit_loss_sum = crit_loss.sum()
				mean_iteration_critic_loss += (crit_loss_sum / crit_repeats)
				# Update gradients
				crit_loss_sum.backward(retain_graph=True)
				# Update optimizer
				crit_opt.step()
				gc.collect()
			critic_losses += [mean_iteration_critic_loss]
			#print("critic_losses: "+str(getsizeof(critic_losses)))
			...
			#print("generator loses: "+str(getsizeof(generator_losses)))
	...
	# Save Generator
	torch.save(gen.state_dict(), "treegen3")
	torch.save(crit.state_dict(), "treecrit3")

	from torchvision import transforms
	fake_noise = get_noise(64, 64, device=device)
	fake = gen(fake_noise)
	show_tensor_images(fake[1])
	image_unflat = ((fake[1]+1)).detach().cpu()
	image_rgb = abs(torch.ones_like(image_unflat)*255-torch.floor((image_unflat) * (torch.ones_like(image_unflat)*127.5)))
	#image_remove_bg = torch.where(image_rgb < float(10), torch.zeros_like(image_rgb), image_rgb)
	#print(image_rgb)
	from torchvision import transforms
